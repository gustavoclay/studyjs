...
async function cadastrar() {
  ...
  console.log('ADD 1º')
  await axios.post(`${baseUrl}/livro`, {
    titulo: 'Livro TESTE1',
    autor: 'DevSecOps',
...
async function buscarPorID() {
  ...
  await axios.get(`${baseUrl}/livro/${livro2.id}`)
    .then(response => {
...
async function atualizar() {
  ...
  console.log('Update')
  await axios.put(`${baseUrl}/livro/${livro.id}`, {
  ...
  console.log('Busca')
  await axios.get(`${baseUrl}/livro/${livro.id}`).then(response => {
...
async function deletar() {
  ...
  console.log('Delete')
  await axios.delete(`${baseUrl}/livro/${livro.id}`)
    .then(response => {
      console.log('✅ Pass')
    })
    .catch(error => {
...
async function validarBuscarPorID() {
  ...
  await axios.get(`${baseUrl}/livro/${livro.id}`)
    .then(response => {
      console.log('❎ Fail')
    })
    .catch(error => {
...
async function validarAtributosAtualizar() {
  ...
  await axios.put(`${baseUrl}/livro/${livro2.id}`, {
    ano: 2000,
    preco: 100.00
  })
    .then(response => {
...
async function buscarPorAutor() {
  ...
  await axios.get(`${baseUrl}/livro/autor/DevSecOps`)
    .then(response => {
      if (response.data.length > 0) {
...
async function media() {
  ...
  const res = await axios.get(`${baseUrl}/livro`)

  const media = res.data.reduce((soma, li) => soma + Number(li.preco), 0) / res.data.length

  await axios.get(`${baseUrl}/livro/preco/media`)
    .then(response => {
